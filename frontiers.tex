...
Viruses, particularly RNA viruses, exhibit high genetic divergence and possess
compact genomes, making them more suitable for these types of molecular studies
compared to bacteria. The high evolutionary rate of RNA viruses, compared to
bacterial DNA, allows the detection of genetic fingerprints in relatively short
time frames. These fingerprints can be targeted for epidemiological studies
over short time periods \cite{paraskevis_application_2016}. The selection of a
genetic distance threshold becomes particularly important in this context.
Depending on the disease and its context of spread, the threshold may vary. For
instance, a highly contagious respiratory illness may require a smaller genetic
distance threshold than a less contagious illness primarily spread through
direct contact.
...
network analysis. By autonomously optimizing clustering metrics derived from
pairwise genetic distances, AUTO-TUNE has the potential to improve the accuracy
and reliability of network inference, irrespective of epidemic context and data
...
Our analysis demonstrates its potential in augmenting public health responses,
while also acknowledging the need for careful interpretation of its results,
especially in cases with no clear optimal threshold. 
...
\begin{enumerate}
	...
	\item{ A priority score is assigned to each $d_L$. This score measures two properties of the threshold: Does $R_{12}$ jump at $d_L$? How far is the number of clusters $C$ at $d_L$ from the maximal number of clusters over all threshold values? Let there be $N$ overall $d_L$ candidate values, and assume we are examining the ith candidate, $d_L^i$ with $W < i \leq N - W$ ($W$ is a positive integer defined below).
	            ...
	            \begin{enumerate}
		            \item{The $R_{12}$ jump is computed by looking at the normalized ratio of the mean $R_{12}$ values computed over the leading window $d_L^{i+1}…d_L^{i+W}$ and the trailing window $d_L^{i-W}… d_L^{i-1}$. The width of the window, $W$, is defined as $\min \left( \max \left(\left[\frac{N}{100}\right], 3\right), 30 \right)$.
