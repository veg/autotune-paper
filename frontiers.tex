...
First, (here I consider we should mention the what you did for clustuneR Comparison
...
Furthermore, we aimed to evaluate the potential impact of sampling density on the genetic distance threshold determined by AUTO-TUNE. To accomplish this, we randomly subsampled the original data set of one of the studies up to 10 times, considering proportions of $25\%$, $50\%$, and $75\%$ of the original sample size. For each subset, we calculated the optimal threshold using AUTO-TUNE. For this analysis we only used (the Standford) data set which collected (number) samples between (years) due to (explain if you picked that one for something in particular).
