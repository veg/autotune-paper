...
Choosing an appropriate distance threshold is an important part of using a
genetic transmission network to track the spread of a infectious disease
\cite{liu_dynamics_2020,rose_persistence_2020}. This distance threshold
determines how close two individuals must be in order for a link to be created
between them in the network. 
Using a distance threshold that is too large can result in a network with many
uninformative links, making it difficult to interpret and analyze. On the other
hand, using a distance threshold that is too small can result in a network with
too few links, underestimating connections between individuals and making it
difficult to accurately track the spread of the infectious pathogen\cite{gore_hiv_2022}.

To ensure that the transmission network is useful and informative, it is
important to carefully consider the appropriate distance threshold. This may
vary depending on the specific disease and the context in which it is spreading.
For example, a highly contagious respiratory pathogen may require a smaller
distance threshold than a less contagious pathogen that is primarily spread
through direct contact. Viruses are more amenable to molecular studies compared
with bacteria due to their high genetic divergence and compact genomes. Given the
relatively high evolutionary rate of RNA viruses detectable genetic fingerprints can be targeted for epidemiological
studies over short time periods\cite{paraskevis_application_2016}. 


For long-standing infections such as HIV, the most appropriate genetic distance
threshold should be determined according to characteristics of the epidemic such
as the speed of transmission, and the mutation rate of the gene region analyzed
\cite{liu_dynamics_2020}. Also, sampling density and possible delays between
...
Clusters analysis in public health may have several purposes such as early
...
In general, the goal is to strike a balance between having enough links to
accurately track the spread of the disease, while not having so many links that
spurious connections are made and hinder the public health response. This balance can be
...
The software implementation involves a step-by-step process that utilizes the HIV-TRACE suite of packages [CITE PAPER]. It starts with calculating pairwise distances wth the tn93 tool and a supplied multiple sequence alignment. 
...
The results of our study suggest that AUTO-TUNE, which relies
solely on clustering metrics from pairwise distances, could be an effective
alternative to other distance-based methods, such as clustuneR while less time
consuming and possessing a gentle learning curve, which makes it easy to use by
...
where, in addition to calculation of  genetic distances, it must also determine
...
Tennessee, Seattle, and Northern Alberta) that outperformed clustuneR using DWH
on 3-year collection date windows across all three datasets. This indicates
...
The AUTO-TUNE methodology may have several limitations. First, even though it
provides the advantage of operating without the need for metadata, the size and
the subgenomic region analysed may affect the accuracy of transmission
